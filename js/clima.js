const URL_CLIMA = "https://api.open-meteo.com/v1/forecast?latitude=-0.2298&longitude=-78.5258&hourly=temperature_2m";
...
async function obtenerClimaQuito() {
    try {
        mensajeClima.textContent = "Conectando con satelites metereologicos...";
        contenedorClima.innerHTML = "";
        let respuesta = await fetch(URL_CLIMA);
        let datosConvertidos = await respuesta.json();
        let climaActual = datosConvertidos.current_weather;
        ...
        if(climaActual.weathercode>=61 && climaActual.weathercode<=65) estadoCielo="Lluvia ligera a moderada";
        if(climaActual.weathercode>=66 && climaActual.weathercode<=67) estadoCielo="Lluvia helada";
        if(climaActual.weathercode>=71 && climaActual.weathercode<=77) estadoCielo="Nieve ligera a moderada";
